...
import { WorkerJob } from '../jobqueue/queue/imputation.queue';
...
import { ImputationModel } from '../jobs/models/imputation.model';
...
const processorFile = path.join(__dirname, 'imputation.worker.js');
...
export const createWorkers = async (
  ...
  for (let i = 0; i < config.numWorkers; i++) {
    ...
    worker.on('completed', async (job: Job, returnvalue: any) => {
      ...
      const imputationFile = `${pathToOutputDir}/imputation.txt`;
      const plotsFile = `${pathToOutputDir}/plot.png`;

      //update db with result files
      const finishedJob = await ImputationJobsModel.findByIdAndUpdate(
        job.data.jobId,
        {

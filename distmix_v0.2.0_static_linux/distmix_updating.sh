...
##### Parameters
gwas_summary=$4;    # GWAS Summary statistics
chromosome=$5;      # positive integer between 1 and 22 or string such as "1p” or “2q"
...
function error_exit(){
  printf " Your input file should contain at least 6 columns";
exit 1;
}
...
if [[ ! -f "${input_dir}/$gwas_summary" ]]; then
...
read -r line < "${input_dir}/$gwas_summary"     ## read first line from file into 'line'
...
if [[ $z -eq "" ]];
then
  Rscript --vanilla z_estimates.R ${input_dir}/$gwas_summary
fi
...
then
  if [[ $chr != "" ]];
  then
  distmix -c $chr  ${input_dir}/$gwas_summary -o ${output_dir}/$output -r $Reference_Panels -i $Reference_Index
  exit 1;
fi
distmix  ${input_dir}/$gwas_summary -o ${output_dir}/$output -r $Reference_Panels -i $Reference_Index
fi
...
then
  if [[ "$chr" != "" ]];
  then
  distmix -c $chr ${input_dir}/$gwas_summary -o ${output_dir}/$output -r $Reference_Panels -i $Reference_Index -w $Populations_Weight
  exit 1;
   fi
distmix  ${input_dir}/$gwas_summary -o ${output_dir}/$output -r $Reference_Panels -i $Reference_Index -w $Populations_Weight
fi
